...
export default function ToolsView() {
  ...
  useEffect(() => {
      ...
      listen<string>('mcp-log', (event) => {
          setTestLogs(prev => [...prev, event.payload]);
      }).then(unl => {
          unlisten = unl;
      });
  ...
  const handleBuildTool = async () => {
      ...
      setIsBuilding(true);
      try {
          const generatedContent = await invoke<string>('build_tool', { prompt, vaultPath });
          
          try {
              const config = JSON.parse(generatedContent);
              if (selectedTool.type === 'mcp') {
                  if (config.name) setEditName(config.name);
                  if (config.command) setEditCommand(config.command);
                  if (config.args) setEditArgs(config.args.join('\n'));
                  if (config.env) {
                      setEditEnv(Object.entries(config.env).map(([k,v]) => `${k}=${v}`).join('\n'));
                  }
              } else {
                  setEditCode(generatedContent);
              }
          } catch (e) {
              ...
              if (selectedTool.type !== 'mcp') {
                  ...
                  alert("AI generated invalid JSON config.");
              }
          }
          setPrompt('');
      } catch (e) {
          console.error("Failed to build tool config", e);
          alert("Failed to build tool config: " + e);
      } finally {
          setIsBuilding(false);
      }
  };
  ...
  const handleTestMcpServer = async () => {
      ...
      try {
          ...
          const result = await invoke<{tools: {name: string, description?: string}[]}>('ping_mcp_server', {
          ...
          setTestResult({ success: true, message: `Connected! Found tools: ${names}` });
      } catch (e) {
          console.error("Failed to ping MCP server", e);
          setTestResult({ success: false, message: `Connection failed: ${e}` });
      } finally {
          setIsTesting(false);
      }
  };
                    ...
                    {selectedTool.id === '' && (
                        <div style={{ marginBottom: '32px', padding: '24px', background: 'var(--background-secondary)', borderRadius: '8px', border: '1px solid var(--background-modifier-border)' }}>
                            ...
                            <textarea 
                                ...
                                placeholder={`Describe the ${selectedTool.type === 'mcp' ? 'MCP server' : 'local tool'} you want to configure...`}
                                style={{
                                    ...
                                    resize: 'none', color: 'var(--text-normal)', marginBottom: '16px'
                                }}
                            />
                            <button 
                                className="btn"
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                        ...
                        {testResult && (
                            <div style={{ 
                                ...
                                border: `1px solid ${testResult.success ? 'var(--text-success)' : 'var(--text-error)'}`,
                                color: testResult.success ? 'var(--text-success)' : 'var(--text-error)',
                                marginTop: '16px'
                            }}>
                                {testResult.success ? <CheckCircle2 size={20} style={{ flexShrink: 0, marginTop: '2px' }} /> : <XCircle size={20} style={{ flexShrink: 0, marginTop: '2px' }} />}
