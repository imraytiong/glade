...
export default function TracePanel() {
  ...
  useEffect(() => {
    console.log("TracePanel useEffect started");
    const unlistenPromise = listen<TraceEvent>('glade://agent-trace', (event) => {
      console.log("TracePanel received event:", event);
  ...
  const handleApproval = async (id: string, approved: boolean, logId: string) => {
    try {
      await invoke('approve_agent_action', { id, approved });
      // Update UI to remove approval buttons
      setLogs(prev => prev.map(log => {
        if (log.id === logId) {
          return { ...log, content: log.content + (approved ? ' (Approved)' : ' (Denied)'), pendingApproval: undefined };
        }
        return log;
      }));
    } catch (e) {
          ...
          <div key={log.id} style={{ 
            ...
            {log.pendingApproval && (
              <div style={{ marginTop: '8px', display: 'flex', gap: '8px' }}>
                <button 
                  className="btn" 
                  style={{ background: 'var(--text-success)', color: 'var(--bg-primary)', border: 'none', padding: '4px 12px', borderRadius: '4px', cursor: 'pointer' }}
                  onClick={() => handleApproval(log.pendingApproval!.id, true, log.id)}
                >
                  Approve
                </button>
                <button 
                  className="btn" 
                  style={{ background: 'var(--text-error)', color: 'var(--bg-primary)', border: 'none', padding: '4px 12px', borderRadius: '4px', cursor: 'pointer' }}
                  onClick={() => handleApproval(log.pendingApproval!.id, false, log.id)}
                >
                  Deny
                </button>
              </div>
